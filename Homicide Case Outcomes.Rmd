...
Given the disproportionate homicide rates by both race and gender, this paper examines whether the outcome (or disposition) of polices’ homicide cases were impacted by a victim’s sex or race/ethnicity. 
...
Cases that involved female homicide victims’ cases, on average, were more likely to have dispositions of ‘closed by arrest’ than cases that involved male victims. Additionally, these graphs show how race plays a role in case dispositions. 
```{r fig.align="center", out.width = "600px"}
library(png) 
library(grid)
...
These findings are important and call attention to the fact that black or African American individuals represent approximately 63 percent of all homicide victims. They also show that homicide cases that involve a female victim are significantly more likely to have their case closed by an arrest than cases with a male victim. Despite this, the most pressing and sobering finding may be that approximately 50 percent of all homicide cases in this dataset were not closed by an arrest. 

Bettering the relationship between the police and the public is vital in attempting to address this issue. Policing policies and programs must be altered in an attempt to increase trust between authorities and the public. This is not a call for an increase in police presence in urban areas. After all, as one study concluded, increased numbers of arrests are not correlated with and do not directly affect homicide rates (Rosenthal & Wallman, 2019). Additionally, the data for this paper were gathered from 55 American cities; areas with well-established police departments. 
